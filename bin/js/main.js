...
Main.__name__ = true;
Main.main = function() {
	new sys_net_Socket().connect(new sys_net_Host("localhost"),5000);
...
sys_NodeSync.callMany = function(f) {
	var retArgs = null;
	f(Reflect.makeVarArgs(function(args) {
		retArgs = args;
	}));
...
var sys_net_Host = function(name) {
	...
	if(ret[0] != null) {
		throw haxe_Exception.thrown(ret[0]);
	}
	this.host = name;
	this.ipStr = ret[1];
	var _g = [];
	...
	while(_g1 < _g2.length) _g.push(Std.parseInt(_g2[_g1++]));
	this.ip = _g[0] << 24 | _g[1] << 16 | _g[2] << 8 | _g[3];
};
...
sys_net_SocketInput.prototype = $extend(haxe_io_Input.prototype,{
	...
	,readBytes: function(buf,pos,len) {
		...
		while(len > 0) {
			...
			var avail = buf.length - this.s.inputPos;
			if(avail > len) {
				buf.copy(nbuf,pos,this.s.inputPos,this.s.inputPos + len);
				pos += len;
				this.s.inputPos += len;
				break;
			}
			buf.copy(nbuf,pos,this.s.inputPos,this.s.inputPos + avail);
			pos += avail;
			len -= avail;
			this.s.inputData.shift();
			this.s.inputPos = 0;
